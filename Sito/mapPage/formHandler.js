...
function eventFormHandler() {
    // Recupera i dati dai campi del form
    const eventName = document.getElementById('nomeEvento').value;
    const eventDate = document.getElementById('dataEvento').value;
    const eventLocation = document.getElementById('luogoEvento').value;
    const eventType = document.getElementById('tipologiaEvento').value;
    const eventParticipants = document.getElementById('numeroPartecipanti').value;
    const eventDescription = document.getElementById('descrizioneEvento').value;
    ...
    // Validazioni
    if (!eventName || !eventDate || !eventLocation || !eventType || !eventParticipants || !eventDescription) {
        console.error("Tutti i campi sono obbligatori.");
        return;
    }
    ...
    // Step 1: Carica l'immagine su Cloudinary
    fetch('http://localhost:3000/generate-signed-url-Eventi', {
        method: 'POST',
        headers: {
            'Authorization': `Bearer ${tokenFromStorage}`
        }
        ...
        .then(data => {
            ...
            const eventData = {
                nome: eventName,
                data_inizio: eventDate,
                luogo: eventLocation,
                posizione: {latitudine: 0, longitudine: 0}, // Aggiorna se necessario
                id_categoria: "673603662b45400acaf456c7",  // Passa il valore direttamente
                numero_massimo_partecipanti: parseInt(eventParticipants, 10),
                descrizione: eventDescription,
                foto: imageUrl
            };

            console.log("Dati dell'evento:", eventData);

            // Chiamata API per creare l'evento
            return fetch('http://localhost:3000/api/eventi', {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${tokenFromStorage}`,
                    'Content-Type': 'application/json'  // Invia come JSON
                },
                body: JSON.stringify(eventData)  // Corpo della richiesta in JSON
            });
        })
        .then(response => response.json())
        .then(data => {
