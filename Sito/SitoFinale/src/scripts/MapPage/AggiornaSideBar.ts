...
export async function estraiDati(lat: number, lng: number, rad: number): Promise<SideCard[]> {
    const posts: SideCard[] = [];
    const eventi: SideCard[] = [];
    const textuals: SideCard[] = [];
    const parties: SideCard[] = [];
...
// Funzione per estrarre i party
async function estraiPartyDaFile(lat: number, lng: number, rad: number, parties: SideCard[]) {
    try {
...
async function estraiPostDaFile(lat: number, lng: number, rad: number, posts: SideCard[], textuals: SideCard[]) {
    try {
        const payload = { lat, lng, rad };
...
async function estraiEventiDaFile(lat: number, lng: number, rad: number, eventi: SideCard[]) {
    try {
        const payload = { lat, lng, rad };
